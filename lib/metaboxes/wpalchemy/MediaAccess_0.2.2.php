 ...
 class WPAlchemy_MediaAccess
{
    ...
    //WPAlchemyのアップ済み画像再利用時に挿入ボタンが表示されない
    //WPデフォルトのエディターなどを呼び出す際は問題ないが・・・
		//return 'media-upload.php?post_id=' . $post_ID . '&tab=' . $tab . '&TB_iframe=1';
		
		//とりあえずこれに変更で挿入ボタンは呼び出しできたが・・・様子を見よう
		return 'media-upload.php?post_id=0&type=image&TB_iframe=true';
	}
